...
def get_gs_client() -> gspread.Client:
    ...
    Даем права на чтение/запись (нужно, чтобы сохранять индивидуальные коды).
    """
...
async def cmd_debug(message: Message):
    """
    Проверяем доступ к таблице + показываем заголовки и примеры email + коды доступа.
    """
    try:
...
async def handle_email(message: Message):
    ...
    if waiting_email.get(user_id):
        ...
        # Клавиатура: открыть уроки + написать Артёму
        ...
        # Основное сообщение в стиле PPE*
        await message.answer(
            "🧰 *PPE* ACCESS\n\n"
        ...
        # Отдельное короткое сообщение только с кодом — удобно копировать
        await message.answer(
